...
import InstitucionesPage from './components/InstitucionesPage';
...
export default function App() {
  const [pagina, setPagina] = useState<'home' | 'instituciones' | 'estudiantes'>('home');
  ...
  const consultarDetalle = async () => {
    if (!estudianteId) return alert('Ingresa un ID de Alumno');
    setLoading(true);
    try {
      ...
      alert('No se encontro actividad academica para el ID: ' + estudianteId);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main className="font-['Quicksand'] bg-green-600 w-full min-h-screen py-10 px-4 m-0">
      <div className="max-w-5xl mx-auto">
        <nav className="bg-white/95 backdrop-blur-sm rounded-full px-4 py-3 shadow-2xl mb-8 flex items-center justify-between">
          <span className="font-black text-green-700 text-lg">EduGrade Global</span>
          <div className="flex gap-2">
            <button
              type="button"
              onClick={() => setPagina('home')}
              className={`px-5 py-2 rounded-full font-bold transition-all ${
                pagina === 'home' ? 'bg-green-500 text-white' : 'bg-slate-100 text-slate-700'
              }`}
            >
              Home
            </button>
            <button
              ...
              className={`px-5 py-2 rounded-full font-bold transition-all ${
                pagina === 'instituciones' ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-700'
              }`}
            >
              Alta Instituciones
            </button>
            <button
              ...
              className={`px-5 py-2 rounded-full font-bold transition-all ${
                pagina === 'estudiantes' ? 'bg-cyan-500 text-white' : 'bg-slate-100 text-slate-700'
              }`}
            >
              Alta Estudiantes
            </button>
        ...
        {pagina === 'instituciones' ? (
          <InstitucionesPage onBackHome={() => setPagina('home')} />
        ) : pagina === 'estudiantes' ? (
          <EstudiantesPage onBackHome={() => setPagina('home')} />
        ) : (
          <>
            <header className="text-center mb-10">
              <h1 className="text-4xl font-black text-white mb-6 tracking-tight drop-shadow-md">EduGrade Global</h1>
              <div className="flex flex-wrap justify-center gap-4">
                ...
                >
                  {loading ? '...' : `Pais Top: ${nombreMejorPais || 'Ver'}`}
                </button>
              </div>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <section className="bg-white/95 backdrop-blur-sm p-8 rounded-[2.5rem] shadow-2xl">
                ...
                <form onSubmit={handleRegistrar} className="space-y-4">
                  ...
                  <input className="w-full border-2 border-slate-100 p-4 rounded-2xl" placeholder="ID Institucion" value={institucionId} onChange={e => setInstitucionId(e.target.value)} />
            ...
            {resultado && (
              <div className="mt-10 bg-slate-900 text-white p-10 rounded-[3rem] text-center shadow-2xl border-4 border-slate-800">
                <p className="text-emerald-400 font-bold uppercase tracking-widest text-sm mb-2">Equivalencia Sudafrica</p>
